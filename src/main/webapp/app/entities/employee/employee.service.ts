...
export class EmployeeService {
    ...
    private convertDateFromClient(employee: IEmployee): IEmployee {
        const copy: IEmployee = Object.assign({}, employee, {
            startDate: employee.startDate != null && employee.startDate.isValid() ? employee.startDate.format(DATE_FORMAT) : null,
            endDate: employee.endDate != null && employee.endDate.isValid() ? employee.endDate.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((employee: IEmployee) => {
            employee.startDate = employee.startDate != null ? moment(employee.startDate) : null;
            employee.endDate = employee.endDate != null ? moment(employee.endDate) : null;
        });
        return res;
    }
}
